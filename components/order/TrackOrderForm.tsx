  ...
  return (
    <div>
      <form onSubmit={onSubmit} className="grid gap-3 rounded-2xl border border-line bg-white p-4 shadow-lift">
        ...
        <button disabled={loading} className="focus-ring h-12 rounded-xl bg-primary px-4 text-sm font-semibold text-white shadow-crisp transition hover:bg-primaryDark disabled:opacity-70">
          {loading ? "Checking..." : "Track order"}
        </button>
      </form>
      {error ? <div className="mt-4 rounded-xl border border-dangerText/20 bg-dangerBg p-3 text-sm font-medium text-dangerText">{error}</div> : null}
