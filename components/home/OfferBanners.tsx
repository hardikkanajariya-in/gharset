      ...
      <Container>
        <div className="grid gap-3 md:grid-cols-2">
          {offers.map((offer) => (
            ...
            >
              {offer.badge ? (
                <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-primaryDark">
                  {offer.badge}
                </span>
              ) : null}
              <p className="mt-3 text-lg font-black tracking-tight">{offer.title}</p>
              <p className="mt-1 text-sm font-medium leading-6 text-blue-100">{offer.description}</p>
              <span className="mt-3 inline-flex text-xs font-black text-white transition group-hover:translate-x-1">
                View offer
              </span>
            </Link>
          ))}
