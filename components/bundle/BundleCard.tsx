    ...
    <article className="overflow-hidden rounded-2xl border border-line bg-white shadow-soft transition hover:-translate-y-0.5 hover:border-lineStrong hover:shadow-lift">
      <div className="border-b border-line bg-primaryDark p-4 text-white">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-blue-100">Solution bundle</p>
            <h3 className="mt-2 text-lg font-semibold text-white">{bundle.name}</h3>
          </div>
          <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-primaryDark">
      ...
      <div className="p-4">
        <div className="grid grid-cols-3 gap-2">
          {products.slice(0, 3).map((product) => (
            <div key={product.productId} className="rounded-xl border border-line bg-mutedSurface px-2 py-3 text-center text-[10px] font-semibold leading-tight text-mutedStrong">
              {product.subcategory || product.category.replace(" Organizers", "")}
            </div>
          ))}
        </div>
        <p className="mt-4 text-sm font-semibold text-ink">Includes</p>
        ...
        <div className="mt-4 flex flex-wrap items-baseline gap-2">
          ...
          {bundle.mrp ? <span className="text-xs font-medium text-muted line-through">{formatPrice(bundle.mrp)}</span> : null}
          {saving ? <span className="rounded-full bg-accentSoft px-2 py-1 text-xs font-semibold text-accent">Save {formatPrice(saving)}</span> : null}
        </div>
